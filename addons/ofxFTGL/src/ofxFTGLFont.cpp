...
void ofxFTGLFont::loadFont(string filename, float fontsize, bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours){
    ...
    if(!font->FaceSize(fontsize)) printf("Failed to set size");
...
ofRectangle ofxFTGLFont::getStringBoundingBox(string s, float x, float y){
    ...
    return ofRectangle(x+bbox.Lower().Xf(), ofGetHeight()-y+bbox.Lower().Yf(), bbox.Upper().Xf(), bbox.Upper().Yf());
}
