...
class ofxSVG{
	...
	// ROGER
	int getWidth() { return docWidth; };
	int getHeight() { return docHeight; };
...
private:
	...
	// ROGER
	int seqc;
	ofColor nextSeqColor()
	{
		ofColor c;
		// No more colors!
		// Paint white to alert
		if (seqc >= 256)
		...
		else
		{
			int hue;
			//hue = seqc;
			hue = (seqc/2) + (128 * (seqc%2));
			//hue = (seqc/4) + (64 * (seqc%4));
			c = ofColor::colorFromHue(hue);
		}
		seqc++;
		return c;
	}
