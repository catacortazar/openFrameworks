...
//--------------------------------------------------------------
bool ofxSVG::load(string svgPath){
...
void ofxSVG::parseLayer(){
    ...
    for(int i=0; i<numObjects; i++) {
		...
		if(bVerbose) cout<<"ofxSVG:          obj " << i << ": " << name  << endl;
        ...
        else if(name == "polyline") parsePolygon();		// ROGER
    ...
    if(!(fill=="none" && stroke=="")){
        ...
        if(fill!="none"){
            ...
            if(fill!=""){
				...
				float g = (rgb >> 8) & 0xFF;
				float b = (rgb) & 0xFF;
#ifdef SEQUENTIAL_COLORS
				ofSetColor(this->nextSeqColor(),alpha);
#else
