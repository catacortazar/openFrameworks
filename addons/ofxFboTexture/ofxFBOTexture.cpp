...
void *ofxFBOTexture::getPixels() {
	...
	// FLip texture
	// copy from ofImage
	if (texData.bFlipTexture)
	{
		int sizeOfOneLineOfPixels = texData.width * (texData.glType==GL_RGBA?4:3) ;
		unsigned char * tempLineOfPix = new unsigned char[sizeOfOneLineOfPixels];
		unsigned char * linea;
		unsigned char * lineb;
		for (int i = 0; i < texData.height/2; i++)
		{
			linea = (unsigned char *) ((int)pixels + i * sizeOfOneLineOfPixels);
			lineb = (unsigned char *) ((int)pixels + ((int)texData.height-i-1) * sizeOfOneLineOfPixels);
...
//checks the grand GL_EXTENSIONS string for a token
