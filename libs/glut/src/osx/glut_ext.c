...
glutExtensionSupported(const char *extension)
{
  ...
  /* To correctly handle apps that create/delete multiple contexts
     (possibly malloc'd at the same address), we must query every time. */
...
#if !defined(__APPLE__)
const static struct name_address_pair glut_functions[] = {
...
{
#if defined(__APPLE__)
  return __glutGetGLProcAddress(procName);
#else
   if(procName[0] == 'g' &&
      procName[1] == 'l' &&
      procName[2] == 'u' &&
      procName[3] == 't')
   {
      ...
      for (i = 0; glut_functions[i].name; i++) {
         if (strcmp(glut_functions[i].name+4, procName+4) == 0)
